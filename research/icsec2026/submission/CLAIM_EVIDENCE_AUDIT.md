...
- OBS_C and OBS_P are separate host-captured serial channels without a verified total order. A controller marker assigned a host timestamp before matching payload confirmation therefore makes the cross-channel detection ordering temporally ambiguous. The retained synthetic replay records `CROSS_CHANNEL_DETECTOR_ACTIVATION_ORDER_AMBIGUOUS` and `DETECTION_UNSCORED_TEMPORAL_ORDER_AMBIGUOUS`, leaves detection ineligible and `UNSCORED`, and preserves independently eligible recovery scoring. Before this rule was changed, all 180 raw logs for the 90 frozen primary trials were hash-verified against the recovery ledger and checked from fault-command time to matching activation confirmation; no primary trial contained a mode-specific detector marker in that interval. No empirical denominator or result changes.
