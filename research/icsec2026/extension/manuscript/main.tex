...
The primary campaign evaluates one physical G431/G474 controller/payload-simulator pair; a separate protocol-level extension evaluates two physical F411 pairs under the same F411 configuration. The simulator exposes three commandable behaviors: silence, CRC corruption, and response delay. A host harness confirms the selected behavior on the simulator channel before accepting a controller-side detection event. After a fixed observation interval, the harness restores normal behavior, confirms that restoration, and observes a predefined recovery marker. Every raw serial record retains a host timestamp, exact bytes, and readable rendering. The resulting claim scope is locked before manuscript assembly: quantitative statements are projected from hash-verified frozen summaries, and unsupported extrapolations are excluded.
...
\begin{itemize}
  ...
  \item a descriptive evaluation of nominal behavior, predefined detection/recovery outcomes, host-observed command-to-marker latency, and pair-specific protocol-level reproduction on two separate physical F411 controller/payload pairs under one fixed F411 implementation; and
...
At the evaluated controller revision, a CRC-invalid response increments the rejection counter and returns without clearing the outstanding request. Its deadline can therefore expire and increment the consecutive-timeout count; three consecutive expirations cause OFFLINE. A subsequently accepted valid response clears the outstanding request and timeout streak and can produce recovery. This is a source-level mechanism statement, not an MCU execution-time measurement.
...
\begin{description}
  \item[RQ1:] What payload-link status and counter behavior is observed during the predefined pre- and post-campaign N0 windows and the separate extended nominal observation?
...
After the campaign, a separate 605-s nominal observation used the same status and prohibited-marker checks. It is reported as additional duration-bounded healthy-operation evidence, not as a third primary N0 window, a fault trial, or a reliability estimate.
...
\subsection{Two-Pair F411 Extension}
...
\begin{table}[t]
\caption{Validated nominal observations. Only within-observation deltas are interpreted.}
\label{tab:n0}
\centering
\scriptsize
...
Extended & 605 & 121 & 1760--2960 & 0 & 0 & 0 & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
Both primary N0 windows were valid. Each lasted 65.0~s and contained 13 ONLINE status records. The successful-message counter increased strictly from 1835 to 1955 before the campaign and from 3049 to 3169 after it. The separate 605.0-s observation contained 121 ONLINE records and increased from 1760 to 2960. In all three observations, timeout, CRC, sequence, and recovery counter deltas were zero and the prohibited-marker list was empty (Table~\ref{tab:n0}). Absolute counter levels reflect preceding activity; only within-observation changes are evaluated.
...
A separate BAD\_CRC mechanism check retained both predefined observations. In the short exposure, the raw order was CRC rejection then one timeout, with no OFFLINE transition before confirmed NORMAL restoration. In the sustained exposure, CRC rejection was followed by timeout markers for consecutive counts one and two, then an OFFLINE marker at count three; only afterward was NORMAL restored and recovery observed. This ordering corroborates the inspected control flow, but these two observations are mechanistic characterization rather than additions to the 90-trial denominator.
...
\subsection{Two-Pair F411 Protocol Reproduction}
