...
\begin{itemize}
  \item a confirmation-gated trial protocol that separates requested, activated, detected, restored, and recovered events and applies stage-specific eligibility rather than counting missing confirmation as failure;
...
Mart\'{i}nez \emph{et al.} use AFIT for fine-grained physical pin-level injection and a readout state machine that filters glitches and invalid experiments~\cite{martinez1999}. Van der Kouwe \emph{et al.} distinguish the faults inserted as an input fault load from the subset activated as the output fault load when analyzing injection distortion~\cite{vanderkouwe2014}. In each case, separate payload-endpoint confirmation gates for detector and recovery scoring are not reported in the cited publication.
...
At the communication boundary, Batista \emph{et al.} describe a COTS failure-emulator mechanism that intercepts and changes nanosatellite I$^2$C traffic for MIL/HIL robustness tests~\cite{batista2019}. Concei\c{c}\~ao and Mattiello-Francisco organize interoperability testing into six MIL, SIL, and HIL modules, reuse model-derived tests, and employ that mechanism for bus faults~\cite{conceicao2025}. Together with the Abboush HIL frameworks, these architectures provide signal-, subsystem-, or multi-point observation and test traceability around fault delivery~\cite{abboush2022,abboush2024,batista2019,conceicao2025}; separate or dual-channel observation alone is therefore not our contribution. Our protocol complements rather than replaces general HIL architectures, FEM, or SATS: those architectures provide fault delivery, execution, and monitoring, while separate payload-side confirmation determines whether detector and recovery observations are eligible for scoring. The cited publications do not report confirmation-gated scoring with all requested, activated, detected, restored, and recovered events kept distinct, nor byte-preserving endpoint traces feeding hash-verified frozen summaries. ``Not reported'' is limited to these publications and is not a claim about all prior work.

\begin{table*}[t]
\caption{Verified closest-work gap matrix.}
\label{tab:gap}
\centering
\footnotesize
...
\toprule
Study & FI setting & Input/request vs. activation & Endpoint confirmation gates detection & Restoration confirmation gates recovery & Auditable per-trial event chain & Byte-preserving/hash-verified evidence \\
\midrule
Arlat/FARM~\cite{arlat1990} & Physical FI method & Fault and activation sets & Not reported & Not reported & Not reported & Not reported \\
Mart\'{i}nez/\newline AFIT~\cite{martinez1999} & Pin-level physical FI & Activated-injection state & Not reported & Not reported & Not reported & Not reported \\
Van der Kouwe~\cite{vanderkouwe2014} & Software FI & Input and output fault loads & Not reported & Not reported & Not reported & Not reported \\
Abboush~\cite{abboush2022,abboush2024} & Automotive HIL FI & Not reported & Not reported & Not reported & Not reported & Not reported \\
Batista/FEM~\cite{batista2019} & Bus MIL/HIL FI & Not reported & Not reported & Not reported & Not reported & Not reported \\
Concei\c{c}\~ao/\newline SATS~\cite{conceicao2025} & Bus MIL/SIL/HIL FI & Stimulus and result traceability & Not reported & Not reported & Not reported & Not reported \\
This work & UART HIL FI & Request and confirmed activation & Payload-confirmation gate & NORMAL-confirmation gate & Five retained events & Exact bytes and SHA-256 \\
\bottomrule
\end{tabular}

\vspace{2pt}
\parbox{0.98\textwidth}{\footnotesize Entries state only properties supported by the cited publication. ``Not reported'' means not reported in that publication, not that the underlying capability does not exist.}
\end{table*}
